		...
		switch(args.length) {
			case 1:
				Users.findOne({user_id: message.author.id, server_id: message.guild.id}, (err, res) => {
					...
					if(!res) {
						if(data.lang === "fr") return sendE("Il semblerait que vous n'avez pas de compte.");
						if(data.lang === "en") return sendE("It seems you don't have an account.");
					};
			...
			case 2:
				...
				Users.findOne({user_id: mUser.id, server_id: message.guild.id}, (err, res) => {
					...
					if(!res) {
						if(data.lang === "fr") return sendE("Cette utilisateur n'a pas de compte.");
						if(data.lang === "en") return sendE("This user does not have an account.");
					};

					if(data.lang === "fr") {
						embed.setAuthor("Compte en banque de " + message.author.tag, message.author.avatarURL).setDescription(mUser.username + " à " + res.money + settings.currency + " sur son compte.");
					};
					if(data.lang === "en"){
						embed.setAuthor("Money of " + message.author.tag, message.author.avatarURL).setDescription(mUser.username + " have " + res.money + settings.currency + " on his account.");
					};
